...
export const summarizeById = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const result = await featuresService.summarizeContent(pageContent.text);
    res.status(200).json(result);
  }
);
...
export const analyzeById = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id || req.body.id;
  console.log(id);
